...
#define CellIdentify @"SOClassTableViewCell"

@interface SOClassViewController ()<UITableViewDataSource,UITableViewDelegate>{
    NSArray *allList;
}
...
@implementation SOClassViewController
...
-(void)headerRefresh:(id)sender{
    [SONetwork getWithProtocol:GET_CLASS_PROTOCOL(self.schoolModel.school_id) andParam:nil success:^(id data) {
        [self.refreshHeader endRefreshing];
        allList = [SOClassModel mj_objectArrayWithKeyValuesArray:data];
        if (allList.count!=0) {
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    SOClassTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentify];
