...
#define CellIdentify @"SOCoachTableViewCell"
@interface SOCoachViewController ()<UITableViewDataSource,UITableViewDelegate>{
...
@implementation SOCoachViewController
...
-(void)headerRefresh:(id)sender{
    ...
    [SONetwork getWithProtocol:GET_COACH_PROTOCOL(self.schoolModel.school_id,pageNo,@"15") andParam:nil success:^(id data) {
        [self.refreshHeader endRefreshing];
        allList = [[NSMutableArray alloc]initWithArray:[SOCoachModel mj_objectArrayWithKeyValuesArray:data]];
        if (allList.count!=0) {
...
-(void)footerRefresh:(id)sender{
    ...
    [SONetwork getWithProtocol:GET_COACH_PROTOCOL(self.schoolModel.school_id,pageNo,@"15") andParam:nil success:^(id data) {
        ...
        NSArray *temp = [SOCoachModel mj_objectArrayWithKeyValuesArray:data];
        if (temp.count!=0) {
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    SOCoachTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentify];
    [cell setCellDetailWithObj:[allList objectAtIndex:indexPath.row]];
    return cell;
}
